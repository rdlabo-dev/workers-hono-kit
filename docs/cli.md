...
| `workers-mysql-db-baseline [--migrations ./drizzle]`    | Brownfield first-deploy helper from `@rdlabo/workers-mysql`: record the baseline `0000` migration as _already applied_ on an existing MySQL DB without running its DDL (the CLI wrapper around `baselineMigrations` / `readBaselineEntry`). Reads DB credentials from `DB_SECRET` (AWS RDS managed secret) or the individual `DB_*` env vars. |
| `workers-hono-kit-db-baseline [--migrations ./drizzle]` | Deprecated compatibility alias for `workers-mysql-db-baseline`.                                                                                                                                                                                                                                                                               |
...
```bash
workers-mysql-db-baseline --migrations ./drizzle
```

Existing scripts may continue to call `workers-hono-kit-db-baseline` during migration, but new
integrations should use the command owned by `@rdlabo/workers-mysql`.
