...
// npm invokes this version hook after bumping the private root, before committing/tagging.
...
const paths = [rootPath, ...packagePaths, lockPath];
const original = paths.map((path) => readFileSync(path, 'utf8'));
const [root, kit, timezone, mysql, lock] = original.map((text) => JSON.parse(text));
const version = root.version;
if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?$/.test(version)) {
  throw new Error('Expected a release version without build metadata');
}
const packages = [kit, timezone, mysql];
const lockKeys = ['packages/hono-kit', 'packages/timezone', 'packages/mysql'];
const names = ['@rdlabo/workers-hono-kit', '@rdlabo/workers-timezone', '@rdlabo/workers-mysql'];
if (
  lock.lockfileVersion !== 3 ||
  packages.some((pkg, i) => pkg.name !== names[i] || !lock.packages[lockKeys[i]]) ||
  !lock.packages['']
) {
...
for (const [i, pkg] of packages.entries()) {
  pkg.version = version;
  lock.packages[lockKeys[i]].version = version;
  for (const field of ['dependencies', 'devDependencies', 'peerDependencies', 'optionalDependencies']) {
    for (const name of names) {
      if (pkg[field]?.[name] === undefined) continue;
      pkg[field][name] = `^${version}`;
      if (!lock.packages[lockKeys[i]][field]) throw new Error(`Missing lockfile ${field} for ${pkg.name}`);
      lock.packages[lockKeys[i]][field][name] = `^${version}`;
    }
  }
}
lock.packages[''].version = version;
lock.version = version;
const values = [root, ...packages, lock];
if (check) {
